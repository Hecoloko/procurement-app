    ...
    return (
        <div className="flex flex-col h-full">
            <div className="p-4 border-b">
                ...
                <p className="text-sm text-gray-500">Select participants to start a chat.</p>
            </div>
            ...
            <div className="p-4 space-y-3">
                <div className="relative">
                    ...
                    <input 
                        ...
                        className="w-full pl-10 pr-4 py-1.5 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 text-sm"
                    />
                </div>
                ...
                {selectedUsers.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                        {selectedUsers.map(user => (
                            <div key={user.id} className="flex items-center gap-1.5 bg-gray-200 rounded-full pl-2 pr-1 py-0.5">
                                <span className="text-sm font-medium text-gray-700">{user.name}</span>
                                <button onClick={() => handleUserToggle(user.id)} className="text-gray-500 hover:text-gray-800"><XMarkIcon className="w-4 h-4"/></button>
                            </div>
                        ))}
                    </div>
                )}
            ...
            <div className="flex-1 overflow-y-auto border-t">
             ...
             <div className="p-4 bg-gray-50 border-t flex justify-end gap-3">
                <button onClick={onCancel} className="bg-white hover:bg-gray-100 text-gray-700 font-semibold py-2 px-4 rounded-lg border border-gray-300">Cancel</button>
                <button 
