...
const getStatusTheme = (status: OrderStatus) => {
    const themes: { [key in OrderStatus]?: string } = {
        'Pending My Approval': 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30',
        'Pending Others': 'bg-blue-500/20 text-blue-400 border-blue-500/30',
        'Approved': 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30',
        'Needs Revision': 'bg-orange-500/20 text-orange-400 border-orange-500/30',
        'Rejected': 'bg-red-500/20 text-red-400 border-red-500/30',
    };
    return themes[status] || 'bg-gray-700 text-gray-300 border-gray-600';
};

const ApprovalSkeleton = () => (
    <div className="bg-[#1E1E1E] p-5 rounded-2xl mb-4 border border-white/5 animate-pulse">
            ...
            >
                <div className="flex-grow min-w-0 pr-4">
                    <div className="flex justify-between items-start mb-1">
                        <h2 className="font-bold text-white text-lg truncate leading-tight">{order.cartName}</h2>
                        <p className="font-bold text-lg text-white tracking-tight whitespace-nowrap">${order.totalCost.toFixed(2)}</p>
                    </div>
                    <p className="text-xs text-gray-400 mt-1 truncate">
                        <span className="font-medium text-gray-300">{submitterName}</span> &bull; {order.submissionDate}
                    </p>
