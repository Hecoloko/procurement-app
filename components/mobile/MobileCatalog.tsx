...
const MobileCatalog: React.FC<MobileCatalogProps> = ({ products, carts, activeCart, onSelectCart, onUpdateItem }) => {
    ...
    return (
        <div>
            ...
            <div className="sticky top-0 bg-gray-50 py-2 z-10 -mx-4 px-4 mb-4 border-b">
                ...
                <div>
                    <label htmlFor="cart-selector-catalog" className="text-xs font-medium text-gray-600">Adding to cart:</label>
                    <select
            ...
            {activeCart && <div className="space-y-3 pb-20">
                {filteredProducts.slice(0, 50).map(product => {
                    ...
                    return (
                        <div key={product.id} className="bg-white p-3 rounded-lg border flex items-center gap-3">
                            <img src={product.imageUrl} alt={product.name} className="w-16 h-16 object-cover rounded-md flex-shrink-0" />
                            <div className="flex-grow">
                                <p className="font-semibold text-sm text-gray-800">{product.name}</p>
                                <p className="font-bold text-gray-900">${product.unitPrice.toFixed(2)}</p>
                            </div>
