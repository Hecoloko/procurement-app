...
const StatCard: React.FC<StatCardProps> = ({ icon: Icon, title, value, onClick }) => (
    <button onClick={onClick} disabled={!onClick} className="glass p-6 rounded-2xl border border-white/20 flex items-center space-x-4 transition-all duration-300 hover:bg-white/40 hover:scale-105 text-left w-full disabled:cursor-default disabled:hover:scale-100 shadow-xl backdrop-blur-md">
...
const ActionButton: React.FC<ActionButtonProps> = ({ icon: Icon, label, onClick }) => (
    <button onClick={onClick} disabled={!onClick} className="glass hover:bg-white/40 active:scale-95 transition-all duration-200 p-6 rounded-2xl border border-white/20 flex flex-col items-center justify-center space-y-3 text-center w-full h-full shadow-xl">
        <div className="bg-white/50 p-4 rounded-full border border-white/20 shadow-sm">
