...
const GlobalCartDrawer: React.FC<GlobalCartDrawerProps> = ({ isOpen, onClose, activeCart, onUpdateItem, onSubmitForApproval, onViewFullCart, onSelectCart, carts }) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 transition-all duration-300 ${isOpen ? 'visible' : 'invisible'}`}>
      ...
      ></div>
      <div
        className={`fixed top-0 right-0 h-full w-full max-w-sm bg-white shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="p-4 border-b flex justify-between items-center bg-gray-50">
          <h2 className="text-xl font-bold text-gray-800">My Active Cart</h2>
        ...
        <div className="p-4 border-b">
          <label htmlFor="cart-selector" className="text-sm font-medium text-gray-700">Select a Cart:</label>
           <select
                id="cart-selector"
                value={activeCart?.id || ''}
          ...
          <>
            ...
            <div className="p-4 border-t bg-gray-50 space-y-3">
              ...
              <button
                ...
                className="w-full bg-white hover:bg-gray-100 text-gray-700 font-bold py-3 px-4 rounded-lg border border-gray-300 transition-colors duration-200 active:scale-95 flex items-center justify-center gap-2"
              >
