...
const getComprehensiveStatusTheme = (status: OrderStatus) => {
    switch (status) {
        ...
        case 'Partially Procured': return 'bg-orange-100 text-orange-800 dark:bg-orange-500/20 dark:text-orange-300 border border-orange-200 dark:border-orange-500/30';
                                ...
                                <tr key={order.id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={() => onSelectOrder(order)}>
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        <span className={`px-3 py-1 text-xs font-semibold rounded-full ${getComprehensiveStatusTheme(order.status)}`}>
                                            {['Partially Procured', 'Completed', 'Processing', 'Shipped'].includes(order.status) ? (
                                                <>
                                                    {order.status}
                                                    <span className="ml-1 opacity-75">
                                                        ({new Set(order.purchaseOrders?.flatMap(po => po.items.map(i => i.id)) || []).size}/{order.items.length})
                                                    </span>
                                                </>
                                            ) : (
                                                order.status
