...
const CartDetail: React.FC<CartDetailProps> = ({ cart, onBack, onOpenCatalog, onManualAdd, onUpdateCartName, onUpdateCartItem, onSubmitForApproval, onRevertToDraft, properties, onOpenEditSchedule, products }) => {
  ...
  const nameParts = cart.name.split('#');
  const cartNumber = nameParts.length > 1 ? `#${nameParts[nameParts.length - 1]}` : '';
  const baseName = nameParts.length > 1 ? nameParts.slice(0, -1).join('#').trim() : cart.name;
  ...
  useEffect(() => {
    const newNameParts = cart.name.split('#');
    const newBaseName = newNameParts.length > 1 ? newNameParts.slice(0, -1).join('#').trim() : cart.name;
    setEditedName(newBaseName);
  }, [cart.name]);
  ...
  const handleNameSave = () => {
    ...
    if (trimmedName && trimmedName !== baseName) {
        const newFullName = `${trimmedName} ${cartNumber}`.trim();
        onUpdateCartName(cart.id, newFullName);
    }
        ...
        <div>
            ...
            <div className="flex items-center gap-2">
              {isEditingName ? (
               ...
               <h1 className="text-3xl md:text-4xl font-bold text-gray-400 tracking-tight">{cartNumber}</h1>
            </div>
